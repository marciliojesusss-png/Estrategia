...
if (-not $TrustServerCertificate) { $TrustServerCertificate = "no" }
...
Write-Host "Migração SQLite -> SQL Server" -ForegroundColor White
...
$migrationOutput = & $python @migrationArgs 2>&1
$migrationText = $migrationOutput -join [Environment]::NewLine
if ($migrationText) {
...
if ($LASTEXITCODE -ne 0) {
    if ($migrationText -match "cadeia de certifica|authority that is not trusted|certificate chain|SSL Provider") {
        ...
        Write-Host ".\migrar-para-sqlserver.bat -Ambiente $Ambiente -TrustServerCertificate yes"
        Write-Host ""
    ...
    throw "Migracao falhou com codigo $LASTEXITCODE."
}
Write-Ok "Migracao concluida"

if ($SyncAuthUsers) {
    Write-Step "Sincronizando usuarios_acesso para SQL Server"
    & $python ".\scripts\sincronizar-usuarios-acesso-sqlserver.py" "--sqlite" $Sqlite
    if ($LASTEXITCODE -ne 0) {
        throw "Sincronizacao de usuarios_acesso falhou com codigo $LASTEXITCODE."
    }
...
if ($GerarSqlAuthUsers) {
    ...
    & $python ".\scripts\gerar-sql-usuarios-acesso.py" "--sqlite" $Sqlite "--database" $Banco
    if ($LASTEXITCODE -ne 0) {
        throw "Geracao do SQL de usuarios_acesso falhou com codigo $LASTEXITCODE."
    }
...
if (-not $SchemaOnly) {
    ...
    $verifyOutput = & $python ".\scripts\verificar-sqlserver.py" "--sqlite" $Sqlite 2>&1
