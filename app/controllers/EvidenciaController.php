...
final class EvidenciaController
{
 ...
 public function download($id){$u=Auth::requirePagePermission('lancamentos','visualizar');try{$e=$this->service->download($id,$u);header('Content-Type: '.$e['tipo_arquivo']);header('Content-Disposition: attachment; filename="'.str_replace('"','',basename($e['nome_arquivo'])).'"');header('X-Content-Type-Options: nosniff');readfile($e['caminho_arquivo']);}catch(Exception$x){ErrorHandler::renderError($x instanceof UnexpectedValueException?403:404);}}
