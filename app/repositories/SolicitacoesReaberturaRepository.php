...
final class SolicitacoesReaberturaRepository
{
    ...
    {
        $sql = 'SELECT * FROM solicitacoes_reabertura WHERE 1=1';
        $params = [];
        if (($filters['status'] ?? '') !== '' && ($filters['status'] ?? '') !== 'Todos') {
            $sql .= ' AND status_solicitacao = :status';
            $params[':status'] = $filters['status'];
        }
        if (($filters['lancamentoId'] ?? '') !== '') {
            $sql .= ' AND lancamento_id = :lancamento_id';
            $params[':lancamento_id'] = (string) $filters['lancamentoId'];
        }
        $sql .= ' ORDER BY data_solicitacao DESC, id DESC';
    ...
    public function replaceAll(array $items): void
    {
        ...
        $this->db->exec('DELETE FROM solicitacoes_reabertura');
