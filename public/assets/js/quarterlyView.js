          ...
          <tbody>
            ${items.map((row) => `
              <tr>
                ...
                ${quarterCells(row)}
                <td><a class="secondary-action table-action dashboard-action" href="${window.AppRoutes ? window.AppRoutes.page("indicadores", { indicadorId: row.indicator.id }) : `/indicadores?indicadorId=${row.indicator.id}`}">Visualizar</a></td>
              </tr>
            `).join("")}
          </tbody>
        </table>
