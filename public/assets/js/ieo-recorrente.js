(function (root) {
  ...
  function isIeoRule(regra) {
    return Boolean(regra) && Number(regra.indicadorId) === INDICADOR_IEO_ID && regra.tipoCalculo === "indice_inverso";
  }
  ...
  function normalizarPercentual(value) {
    if (root.IndicatorFormulas && typeof root.IndicatorFormulas.normalizarPercentual === "function") return root.IndicatorFormulas.normalizarPercentual(value);
  ...
  function parseNumero(value) {
    if (root.CurrencyBR && typeof root.CurrencyBR.parseMoedaBR === "function") return root.CurrencyBR.parseMoedaBR(value);
    if (value === null || value === undefined || value === "") return null;
  ...
  function resultadoBase(unidadeMedida, mensagem, statusCalculo) {
    return {
      resultadoMensal: null, resultadoMensalFormatado: "-", resultadoAcumulado: null, resultadoAcumuladoFormatado: "-",
      resultadoOficialAnual: null, resultadoOficialAnualFormatado: "-", percentualAtingidoMensal: null,
      percentualAtingidoMensalFormatado: "-", percentualAtingidoAcumulado: null, percentualAtingidoAcumuladoFormatado: "-",
      percentualAtingidoAnual: null, percentualAtingidoAnualFormatado: "-", unidadeMedida: unidadeMedida || "percentual",
      statusCalculo: statusCalculo, mensagem: mensagem
    };
  }
  ...
  function calcularIeo(regra, lancamento) {
    ...
    const origemResultado = ieoInformado !== null ? "informado" : "calculado";
    ...
    if (resultado === null) {
      ...
      if (despesaPessoal === null || despesasAdministrativas === null || receitasLiquidas === null || receitasLiquidas === 0) return resultadoBase(regra.unidadeMedida, "Dados insuficientes para cálculo.", "aguardando_dados");
      if (despesaPessoal < 0 || despesasAdministrativas < 0 || receitasLiquidas < 0) {
        const retorno = resultadoBase(regra.unidadeMedida, "Despesa de pessoal, despesas administrativas e receitas líquidas não podem ser negativas.", "erro");
        retorno.erro = true;
        return retorno;
      }
    ...
    if (metaCompetencia === null || metaCompetencia <= 0) return resultadoBase(regra.unidadeMedida, "Meta de referência não cadastrada para a competência.", "aguardando_dados");
    ...
    return {
      resultadoMensal: resultado, resultadoMensalFormatado: formatarPercentual(resultado), resultadoAcumulado: resultado,
      resultadoAcumuladoFormatado: formatarPercentual(resultado), resultadoOficialAnual: resultado,
      resultadoOficialAnualFormatado: formatarPercentual(resultado), percentualAtingidoMensal: percentualAtingido,
      percentualAtingidoMensalFormatado: formatarPercentual(percentualAtingido), percentualAtingidoAcumulado: percentualAtingido,
      percentualAtingidoAcumuladoFormatado: formatarPercentual(percentualAtingido), percentualAtingidoAnual: percentualAtingido,
      percentualAtingidoAnualFormatado: formatarPercentual(percentualAtingido), unidadeMedida: regra.unidadeMedida || "percentual",
      statusCalculo: "calculado", mensagem: "IEO recorrente calculado pela meta da competência e regra inversa.",
      metaReferencia: metaCompetencia, metaReferenciaMensal: metaCompetencia, metaAcumulada: metaCompetencia,
      metaAnualIeo: META_ANUAL_IEO_2026, competenciaReferencia: key, percentualMetaMensal: percentualAtingido,
      percentualMetaAcumulada: percentualAtingido, quantoMenorMelhor: true, sentidoMeta: "quanto_menor_melhor",
      situacao: situacao, ieoRealizadoMes: resultado, ieoApuradoInformado: ieoInformado, origemResultado: origemResultado
    };
  }

  function removerAjusteOficialLegado() {
    if (!root.document) return;
    root.document.querySelectorAll(`[data-entry-field="${CAMPO_PERCENTUAL_OFICIAL_LEGADO}"], [data-entry-field="${CAMPO_OBSERVACAO_AJUSTE_LEGADO}"]`).forEach(function (campo) {
      const bloco = campo.closest(".full-span");
      if (bloco && bloco.querySelector(`[data-entry-field="${CAMPO_PERCENTUAL_OFICIAL_LEGADO}"]`)) { bloco.remove(); return; }
      const label = campo.closest("label");
      if (label) label.remove(); else campo.remove();
    });
  }
  ...
  function instalarCorrecao() {
    ...
    root.IndicatorFormulas.obterRegra = function (indicador, regras) { return ajustarRegraIeo(originalObterRegra.call(this, indicador, regras)); };
    root.IndicatorFormulas.calcularIndicador = function (indicador, regra, lancamentoAtual, lancamentosDoAno) {
      ...
      if (isIeoRule(regraAjustada)) return calcularIeo(regraAjustada, lancamentoAtual, lancamentosDoAno);
  ...
  if (root.document) {
    const iniciar = function () {
      ...
      if (typeof root.MutationObserver === "function") new root.MutationObserver(removerAjusteOficialLegado).observe(root.document.body, { childList: true, subtree: true });
    };
    if (root.document.readyState === "loading") root.document.addEventListener("DOMContentLoaded", iniciar, { once: true }); else iniciar();
  }

  if (typeof module !== "undefined" && module.exports) module.exports = { IEO_META_MENSAL_2026, ajustarRegraIeo, calcularIeo, instalarCorrecao };
