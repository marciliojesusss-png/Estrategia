...
<head>
  ...
  <script src="/assets/js/quarterly.js?v=LUCRO-RECORRENTE-MENSAL-001" defer></script>
  <script src="/assets/js/documentation-fields.js?v=DOCUMENTACAO-CENTRAL-001" defer></script>
  <script src="<?= APP_BASE_PATH ?>/assets/js/indicators.js?v=LUCRO-RECORRENTE-MENSAL-001" defer></script>
...
<body data-page="indicadores">
  <div class="app-shell">
    ...
    <main class="content">
      <section id="indicatorPageHeading" class="page-heading">
        <div>
          <p id="indicatorPageEyebrow" class="eyebrow">Cadastro e consulta</p>
          <h1 id="indicatorPageTitle">Indicadores</h1>
          <p id="indicatorPageDescription">Catálogo metodológico dos indicadores estratégicos.</p>
        </div>
        <button id="backFromIndicatorDetail" class="secondary-action indicator-detail-back" type="button" hidden>Voltar</button>
      </section>
      ...
      <section id="indicatorCatalogPanel" class="panel">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody id="indicadoresTable"></tbody>
          </table>
        </div>
      </section>

      <section id="indicatorDetailPanel" class="panel detail-panel" hidden>
        <div class="detail-header">
          <div>
            <p class="eyebrow">Detalhe do indicador</p>
            <h2 id="detailTitle">Selecione um indicador</h2>
          </div>
          <span id="detailBadge" class="badge info"></span>
        </div>

        <div id="indicatorReadOnly" class="detail-grid"></div>

        <div id="indicatorTracking" class="indicator-tracking">
          <section class="indicator-tracking-section">
            ...
            <div class="table-wrap compact-table indicator-monthly-table table-responsive table-composicao-wrapper">
              <table class="table-composicao-mensal">
                <thead>
                ...
                <tbody id="indicatorMonthlyComposition"></tbody>
              </table>
            </div>
          </section>
          ...
          <section class="indicator-tracking-section">
            <div class="section-heading">
              <div>
                <p class="eyebrow">Consolidado trimestral</p>
                <h3>Trimestres calculados automaticamente</h3>
              </div>
            </div>
            <div class="table-wrap quarterly-detail-table">
              <table>
                <thead>
                  <tr>
                    <th>Trimestre</th>
                    <th>Meses homologados</th>
                ...
                <tbody id="indicatorQuarterlyComposition"></tbody>
              </table>
            </div>
          </section>
        </div>

        <section id="monthlyLaunchDetail" class="monthly-launch-detail" hidden>
          <div class="section-heading">
            <div>
              <p class="eyebrow">Detalhe do lançamento</p>
              <h3 id="monthlyLaunchDetailTitle">Informações prestadas pela unidade apuradora</h3>
              <p id="monthlyLaunchDetailSubtitle" class="section-subtitle"></p>
            </div>
            <button id="backToIndicatorFromLaunch" class="secondary-action table-action" type="button">Voltar ao indicador</button>
          </div>
          <div id="monthlyLaunchDetailContent" class="launch-detail-layout"></div>
        </section>

        <form id="indicatorForm" class="editor-form" hidden>
          <input type="hidden" id="indicatorId">

          <label>Número
            <input id="fieldNumero" type="number" min="1" required>
          </label>

          <label>Indicador
            <input id="fieldIndicador" type="text" required>
          </label>

          <label>Plano
            <select id="fieldPlano" required></select>
          </label>

          <label>Pilar estratégico
            <select id="fieldPilar" required></select>
          </label>

          <label>Periodicidade
            <input id="fieldPeriodicidade" type="text" required>
          </label>

          <label>Unidade apuradora
            <select id="fieldUnidade"></select>
          </label>

          <label>Diretoria responsável
            <select id="fieldDiretoria"></select>
          </label>

          <label>Tipo de cálculo
            <select id="fieldTipoCalculo" required></select>
          </label>

          <label>Unidade de medida
            <select id="fieldUnidadeMedida" required></select>
          </label>

          <label>Status
            <select id="fieldAtivo" required>
              <option value="true">Ativo</option>
              <option value="false">Inativo</option>
            </select>
          </label>

          <label class="full-span">Meta anual
            <textarea id="fieldMetaAnual" rows="3" required></textarea>
          </label>

          <label class="full-span">Métrica/Fórmula de referência
            <textarea id="fieldMetrica" rows="4" required></textarea>
          </label>

          <div class="form-actions full-span">
            <button class="primary-action" type="submit">Salvar cadastro</button>
            <button id="cancelIndicatorEdit" class="secondary-action" type="button">Cancelar</button>
          </div>
        </form>
      </section>
    </main>
