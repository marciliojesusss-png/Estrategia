...
<form method="get" action="<?= e(app_index_url()) ?>" class="filters">
  <input type="hidden" name="route" value="indicadores">
  ...
  <button type="submit">Filtrar</button><a href="<?= e(app_url('indicadores')) ?>">Limpar</a><a href="<?= e(app_url('indicadores/exportar', $filters)) ?>">Exportar CSV</a>
</form>
...
<?php if ($result['pagination']['pages'] > 1): ?><nav aria-label="Paginacao"><?php for ($p=1;$p<=$result['pagination']['pages'];$p++): ?><a href="<?= e(app_url('indicadores', array_merge($filters,array('page'=>$p)))) ?>" <?= $p===$result['pagination']['page']?'aria-current="page"':'' ?>><?= $p ?></a> <?php endfor; ?></nav><?php endif; ?></section>
