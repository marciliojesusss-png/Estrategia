...
$routes=file_get_contents(__DIR__.'/../public/index.php');foreach(array('/api/indicadores/{id}','/api/lancamentos/{id}','/api/homologacoes/{id}','/api/dashboard/{action}','/api/administracao/{resource}','/api/prazos-apuracao')as$route)ok_api(strpos($routes,$route)!==false,'rota canonica ausente: '.$route);
